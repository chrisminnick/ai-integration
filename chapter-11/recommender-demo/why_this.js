...
function historyFor(userId) {
  return (USERS.find(u => u.id === userId)?.interactions || []).map(e => e.itemId);
}
function itemById(id) {
  return ITEM_INDEX.find(x => x.id === id);
}

async function whyThis({ userId, itemId }) {
  ...
  const sys = { role: "system", content: "You are a concise product copywriter." };
  const usr = {
    role: "user",
    content: `Write one short sentence (<= 20 words) explaining why the user might like "${candidateTitle}".
